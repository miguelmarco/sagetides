...
/* --- PARAMETERS  --------------- */
	...
	mpfr_set_str(p[0], "*****", 10, TIDES_RND); 
	mpfr_set_str(p[1], "*****", 10, TIDES_RND); 
	mpfr_set_str(p[2], "*****", 10, TIDES_RND); 

/* --- VARIABLES   --------------- */
	int nvar = 3;
	mpfr_t v[nvar];
	for(i=0; i<nvar; i++) mpfr_init2(v[i], TIDES_PREC);
	mpfr_set_str(v[0], "*****", 10, TIDES_RND); 
	mpfr_set_str(v[1], "*****", 10, TIDES_RND); 
	mpfr_set_str(v[2], "*****", 10, TIDES_RND); 
...
/* --- INTEGRATOR  --------------- */
	mp_tides_delta(lorenzMP, NULL, nvar, npar, nfun, v, p, tini, dt, nipt, tolrel, tolabs, NULL, NULL);
